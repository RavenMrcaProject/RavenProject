...
def main():
    # Parse config to get environment name
    # TODO: change the path to the config file
    config = parse_config(
        "../False_Data_Attacks_on_ORCA/Attack/src/config.txt")
    ...
    env_name = os.path.basename(environment_file.rstrip("/"))
    print("env_name: ", env_name)

    # Copy the YAML file before starting the simulation
    # TODO: change the path to the config file
    base_path = "../comparison/swarmlab/glas_ws/results/singleintegrator/instances"
    source_yaml = os.path.join(base_path, f"{env_name}.yaml")
