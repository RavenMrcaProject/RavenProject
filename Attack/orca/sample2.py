...
pointX = float(config.get("pointX", 0.0))
pointY = float(config.get("pointY", 0.0))
...
# change it to parse_obstacles_onebyone if you want to use it in 3D.
obstacles = parse_obstacles(environment_file + "obstacles.txt")
...
def Orca_Model(state: State, time: Interval, _: Any) -> BasicResult:
    ...
    try:
        ...
        while counter < (totalTimeStep * delay_constant):
            ...
            prev_pos = positions[attackedRobotId]
            ...
            if counter in keys_list_times:
                ...
                # Update positions list with spoofed position
                spoofed_pos = (positions[attackedRobotId][0] + value[0],
                               positions[attackedRobotId][1] + value[1])
                positions[attackedRobotId] = spoofed_pos

            # Calculate next positions using real position
            orca_module.calculate_next_positions(
                time_step,
                positions,  # contains spoofed position
                velocities,
                attackedRobotId,
                prev_pos,   # use real position for calculations
                orca_instance
            )
            """
            ...
            victim_position = positions[victimRobotId]

            for obstacle in obstacles:  # each obstacle is a list of 4 corner points
                # Check distance to each edge of the obstacle
                for i in range(4):
                    # Get line segment points (connect corners, including last to first)
                    point1 = obstacle[i]
                    # wraps back to 0 for last point
                    point2 = obstacle[(i + 1) % 4]

                    # Calculate distance from robot to this edge
                    distance = point_line_distance(
                        victim_position, point1, point2)
            ...
            for updated_position in positions:
            ...
            victim_position = positions[victimRobotId]
            for robot_id, robot_pos in enumerate(positions):
            ...
            dist_victim_point = calculate_abs_distance(pointX, pointY,
                                                       positions[victimRobotId][0], positions[victimRobotId][1])
            # save for falsification
            ...
            last_positions.append(positions[victimRobotId])
            ...
            if calculate_abs_distance(positions[victimRobotId][0], positions[victimRobotId][1], goal_positions[victimRobotId][0], goal_positions[victimRobotId][1]) < 5.0:
                total_pos_change = float('inf')
            ...
            current_position = positions[victimRobotId]
...
if attackType == 1:
    ...
    phi_navigation_delay_of_victim_robot = f"eventually (distToGoal_VictimRobot <= 3.0)"
...
flag = 1
itt = -1
while itt < falsificationRuns and flag:
    itt += 1
    ...
    # with open(f'../False_Data_Attacks_on_ORCA/Attack/src/falsiResults2/{attackName}_run_positions_{falsificationIterations}ite_{numberOfFalseMessage}mes_{environment_name}_{algorithm}_{radius}_{time_step}.txt', 'w') as file:
    ...
    if worst_eval(worst_run_).cost < 0:
        ...
        print("Evaluation cost: ", worst_eval(worst_run_).cost)
        with open(f'../False_Data_Attacks_on_ORCA/Attack/src/falsiResults6/{attackName}_run_positions_{falsificationIterations}ite_{numberOfFalseMessage}mes_{environment_name}_{algorithm}_{radius}_{time_step}.txt', 'w') as file:
            for i in range(0, len(global_log_list_last), num_of_robots + 1):
                time = global_log_list_last[i]
                positions = global_log_list_last[i+1:i+num_of_robots+1]
                positions_str = ' '.join(
    ...
    print("Flag: ", flag)
    print("Iteration: ", itt)
    print("Falsification runs: ", falsificationRuns)
