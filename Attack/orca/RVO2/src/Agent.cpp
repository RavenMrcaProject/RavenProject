...
void Agent::computeNeighbors() {
  ...
  // timeHorizon is 5.0f: Find all obstacles that are 5 timestep away. Orca
  // guarantees no collision within 5 timesteps.
  float rangeSq = sqr(timeHorizonObst_ * maxSpeed_ + radius_);
  // find obstacle neighbors within the rangeQq
  sim_->kdTree_->computeObstacleNeighbors(this, rangeSq);

  // now, find the closest agents
  agentNeighbors_.clear();

  // maxNeighbors_ is 10: Find the closest 10 agents
  if (maxNeighbors_ > 0) {
    // neighborDist_ is 15.0f: Find all agents that are 15.0f away
    rangeSq = sqr(neighborDist_);
    // find agents within this range
    sim_->kdTree_->computeAgentNeighbors(this, rangeSq);
  }
  // at the end of this function call, all neighbor obstacles and agents are
  // efficiently found.
}
...
void Agent::computeNewVelocity() {
  ...
  for (size_t i = 0; i < obstacleNeighbors_.size(); ++i) {
    // closest vertex as obstacle object
    const Obstacle *obstacle1 =
        obstacleNeighbors_[i].second; // distance, closest obstacle vertex
    // ccwise next obstacle vertex
    const Obstacle *obstacle2 =
        obstacle1
            ->nextObstacle_; // the next obstacle vertex in the same obstacle

    // create velocity obstacle given obstacle1
    ...
    for (size_t j = 0; j < orcaLines_.size(); ++j) {
      // center of VO: invTimeHorizonObst * relativePosition1
    ...
    const float distSq1 = absSq(relativePosition1); // squared of distance
    const float distSq2 = absSq(relativePosition2);

    const float radiusSq = sqr(radius_); // square of radius

    const Vector2 obstacleVector =
        obstacle2->point_ - obstacle1->point_; // obstacle vector

    const float s =
        (-relativePosition1 * obstacleVector) / absSq(obstacleVector);
    // s can be +, 0, -
    // distSqLine: distance between two robot position and obstacle segment
...
void Agent::insertAgentNeighbor(const Agent *agent, float &rangeSq) {
  ...
  // agentNeighbors_ is a sorted list
}
...
void Agent::insertObstacleNeighbor(const Obstacle *obstacle, float rangeSq) {
  ...
  // distance of agent to line segment
  const float distSq =
...
void Agent::update(Vector2 spoofed_drone_real_position) {
  velocity_ = newVelocity_;
  spoofed_drone_real_position += velocity_ * sim_->timeStep_;
  position_ = spoofed_drone_real_position;
}
...
bool linearProgram1(const std::vector<Line> &lines, size_t lineNo, float radius,
  ...
  // does robot and line segment intersect?
  if (discriminant < 0.0f) {
    /* Max speed circle fully invalidates line lineNo. */
    // no intersection, no solution found
    return false;
  }
  ...
  // among the feasible segment, left or right is optimal?
  if (directionOpt) {
    /* Optimize direction. */
    ...
    // t: where optVelocity intersects with the line
    const float t =
  ...
  // feasible soultion is found, result is that value
  return true;
}
...
size_t linearProgram2(const std::vector<Line> &lines, float radius,
  ...
  // check if current result satisfies all the lines (constraints)
  for (size_t i = 0; i < lines.size(); ++i) {
    if (det(lines[i].direction, lines[i].point - result) > 0.0f) {
      ...
      if (!linearProgram1(lines, i, radius, optVelocity, directionOpt,
                          result)) {
        // safest possible stays as it is
        result = tempResult;
        return i;
      }
    }
...
void linearProgram3(const std::vector<Line> &lines, size_t numObstLines,
  ...
  // how much robot violates the current constraint
  float distance = 0.0f;

  for (size_t i = beginLine; i < lines.size(); ++i) {
    // if the point 'result' lies to one side of the line
    // the point 'result' is too far away from the line
    if (det(lines[i].direction, lines[i].point - result) > distance) {
      ...
      // how much results violates the current constraint
