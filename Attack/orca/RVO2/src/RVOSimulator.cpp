...
RVOSimulator::RVOSimulator(float timeStep, float neighborDist,
    ...
    : defaultAgent_(NULL), globalTime_(0.0f), kdTree_(NULL),
      timeStep_(timeStep) {
  kdTree_ = new KdTree(this);
...
void RVOSimulator::doStep(int spoofed_robot_id,
                          Vector2 spoofed_drone_real_position) {
  kdTree_->buildAgentTree(); // create kd-tree using agents
  std::cout << "doStep with params is called." << std::endl;
...
#pragma omp parallel for
  ...
  for (int i = 0; i < static_cast<int>(agents_.size()); ++i) {
    agents_[i]
        ->computeNeighbors(); // Dogu: find the nearest obstacles and agents
...
#endif
  for (int i = 0; i < static_cast<int>(agents_.size()); ++i) {
    if (agents_[i]->id_ != spoofed_robot_id) {
      agents_[i]->update(); // Dogu: update the position
    } else {
      agents_[i]->update(spoofed_drone_real_position);
    }
  }

  globalTime_ += timeStep_;

  // print obstacle positions here.
  // for (int i = 0; i < static_cast<int>(obstacles_.size()); ++i) {
  //   std::cout << "Obstacle " << i << " position: " << obstacles_[i]->point_
  //             << std::endl;
  // }
}
